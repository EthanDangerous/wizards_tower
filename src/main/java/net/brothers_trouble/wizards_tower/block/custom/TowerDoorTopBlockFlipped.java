...
public class TowerDoorTopBlockFlipped extends Block {
    private static final VoxelShape CUSTOM_SHAPE = Shapes.box(0, -2, 0, 1, 1, 1); // Custom hitbox

    public TowerDoorTopBlockFlipped(Properties properties) {
        super(properties);
    }
...
//            level.setBlock(pos.above(2), ModBlocks.TOWER_DOOR_TOP.get().defaultBlockState(), 3);
    ...
    public void onRemove(BlockState state, Level level, BlockPos pos, BlockState newState, boolean isMoving) {
        if (!level.isClientSide && state.getBlock() != newState.getBlock()) {
            ...
            if (level.getBlockState(pos.below()).is(ModBlocks.TOWER_DOOR_MIDDLE_FLIPPED.get())) {
                level.destroyBlock(pos.below(), false);
            }
            // Break the top block if it exists
            if (level.getBlockState(pos.below(2)).is(ModBlocks.TOWER_DOOR_BOTTOM_FLIPPED.get())) {
                level.destroyBlock(pos.below(2), false);
            }

            if (level.getBlockState(pos.north()).is(ModBlocks.TOWER_DOOR_TOP.get())) {
                level.destroyBlock(pos.north(), false);
            }
            if (level.getBlockState(pos.below().north()).is(ModBlocks.TOWER_DOOR_MIDDLE.get())) {
                level.destroyBlock(pos.below().north(), false);
            }
            if (level.getBlockState(pos.below().north(2)).is(ModBlocks.TOWER_DOOR_BOTTOM.get())) {
                level.destroyBlock(pos.below().north(2), false);
            }
